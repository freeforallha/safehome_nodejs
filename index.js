  ...
  try {
    ...
    if (
      ...
      await addDeviceNotification(
        ...
        `C?p nh?t| ${(updateData.status ?? oldData.status) === "closed"
          ...
          ? "B? tháo"
          : "Bình thu?ng"
        } | Pin: ${updateData.battery ?? oldData.battery ?? "?"}% | Tín hi?u: ${updateData.linkquality ?? oldData.linkquality ?? "?"}`,
        "heartbeat",
      );
    }
    ...
    if (alarm.enabled) {
      ...
      if (updateData.status && updateData.status !== "closed") {
        await sendAlarm(uid, homeId, "C?a chua dóng");
      }

      if (updateData.tamper === true) {
        await sendAlarm(uid, homeId, "Phát hi?n tháo thi?t b?");
      }
    }
  } catch (err) {
